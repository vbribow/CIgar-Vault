...
export function InventoryManager({ initialItems, catalog, ratings, mode, initialMissing = "all", initialStorage = "all" }: { initialItems: InventoryItem[]; catalog: CatalogCigar[]; ratings:ProfessionalRating[]; mode: DataMode; initialMissing?: string; initialStorage?: string }) {
  ...
  return <>
    ...
    <section className="inventoryMobileList" aria-label="Inventory lots">{filtered.map(item=><article key={item.inventoryId}><div><span>{item.vintage?`Production / release year ${item.vintage}`:"Production / release year needed"} · {item.status||"Review"}</span><h3>{item.brand} {item.line}</h3><p>{item.vitola}</p></div><div className="mobileQuantity"><strong>{item.currentQty??"—"}</strong><span>total cigars</span><small>{item.fullBoxQty??0} box{item.fullBoxQty===1?"":"es"} · {item.looseStickQty??0} loose</small></div><div className="mobileLotActions"><button className="button" onClick={()=>startEditing(item,"quantity")}>Fix quantity</button><button className="button secondary" onClick={()=>startEditing(item,"year")}>{item.vintage?"Edit year":"Add year"}</button><button className="button secondary" onClick={()=>startEditing(item)}>Edit all details</button><a href={`/inventory/${item.inventoryId}`}>Open record →</a></div></article>)}</section>
