...
export function ClimateAlertDashboard({humidors,readings,sensors,inventory,nowISO}:{humidors:Humidor[];readings:HumidorReading[];sensors:EnvironmentalSensor[];inventory:InventoryItem[];nowISO:string}){
  ...
  const riskHumidors=new Set(health.filter(h=>h.severity==="Attention"||h.severity==="Critical").map(h=>h.humidor.humidorId));
  const exposedCigars=inventory.filter(item=>item.storageLocationId&&riskHumidors.has(item.storageLocationId)).reduce((sum,item)=>sum+(item.currentQty??0),0);
  const sustainedCount=health.filter(item=>item.intelligence.sustained).length;
  return <section className="climateCommand" aria-labelledby="climate-command-title">
    <div className="commandHead"><div><div className="eyebrow">Climate command center</div><h2 id="climate-command-title">Protect the collection.</h2><p>Profile-aware status, sustained exposure, recommended action, and collection value in one view.</p></div><div className="windowPicker" aria-label="Climate history range">{Object.keys(windows).map(key=><button className={key===windowKey?"active":""} onClick={()=>setWindowKey(key as keyof typeof windows)} key={key}>{key}</button>)}</div></div>
    <div className="commandMetrics"><article><span>Protected</span><strong>{health.filter(h=>h.severity==="Good").length} / {humidors.length}</strong><small>Humidors reporting in range</small></article><article className={activeAlerts.length?"metricWarning":""}><span>Active alerts</span><strong>{activeAlerts.length}</strong><small>{activeAlerts.filter(a=>a.severity==="Critical").length} critical</small></article><article className={sustainedCount?"metricDanger":""}><span>Sustained exposure</span><strong>{sustainedCount}</strong><small>Humidors with a repeated trend</small></article><article className={exposed?"metricDanger":""}><span>Collection at risk</span><strong>{dollars(exposed)}</strong><small>{exposedCigars} cigars in out-of-range environments</small></article><article className={unmonitored?"metricWarning":""}><span>Unmonitored value</span><strong>{dollars(unmonitored)}</strong><small>Behind offline or missing readings</small></article></div>
    <div className="commandGrid"><div className="healthStack">{health.map(item=>{const chart=item.rows.filter(r=>new Date(r.recordedAt).getTime()>=cutoff).reverse().slice(-40);return <article className={`healthRow ${statusClass(item.severity)}`} key={item.humidor.humidorId}><div className="healthIdentity"><span className={`healthBadge ${statusClass(item.severity)}`}>{item.severity}</span><div><h3>{item.humidor.name}</h3><small>{item.intelligence.profile.label} profile · {item.sensor?`${item.sensor.provider} · ${item.sensor.name}`:"No registered sensor"}</small></div></div><div className="healthNow"><span><small>Temperature</small><strong>{item.latest?`${item.latest.temperatureF}°F`:"—"}</strong></span><span><small>Humidity</small><strong>{item.latest?`${item.latest.humidity}%`:"—"}</strong></span><span><small>Trend</small><strong>{item.intelligence.state}</strong></span><span><small>Stored value</small><strong>{dollars(item.storedValue)}</strong></span></div><div className="miniTrend" aria-label={`${item.humidor.name} readings for ${windowKey}`}>{chart.length?chart.map(r=><i key={r.readingId} className={r.temperatureF<item.humidor.minTempF||r.temperatureF>item.humidor.maxTempF||r.humidity<item.humidor.minHumidity||r.humidity>item.humidor.maxHumidity?"excursion":""} style={{height:`${Math.max(18,Math.min(100,(r.humidity-30)/60*100))}%`}} title={`${r.recordedAt}: ${r.temperatureF}°F · ${r.humidity}%`}/>):<em>No readings in this period</em>}</div>{item.intelligence.consequence&&<div className="climateGuidance"><span>Why it matters</span><p>{item.intelligence.consequence}</p><strong>{item.intelligence.action}</strong></div>}<div className="healthActions"><span>{item.intelligence.summary}</span><a href={`/humidors/${encodeURIComponent(item.humidor.humidorId)}`}>View climate details →</a></div></article>})}{!health.length&&<div className="emptyState">Add a humidor to begin climate monitoring.</div>}</div>
