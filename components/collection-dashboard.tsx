...
export function CollectionDashboard({ collections, inventory, valuations }: { collections: CigarCollection[]; inventory: InventoryItem[]; valuations: Valuation[] }) {
  ...
  return <section className="collectionDashboard"><div className="collectionDashboardHead"><div><div className="eyebrow">Portfolio dashboard</div><h2>Collection value and completeness</h2></div><p>Whole-set values take priority when entered. Component totals use the latest per-cigar valuation multiplied by current quantity.</p></div><div className="collectionDashboardGrid">
    {dashboard.map(({ collection, summary }) => { const peak = Math.max(...summary.valueHistory.map((point) => point.value), 1); return <article className="collectionDashboardCard" key={collection.collectionId}>
      <header><div><div className="eyebrow">{collection.maker || "Collector set"}{collection.releaseYear ? ` · ${collection.releaseYear}` : " · Year needed"}</div><h3>{collection.name}</h3></div><a href="#collection-cards">Review collection →</a></header>
      <div className="collectionDashboardFacts"><div><span>Whole value</span><strong>{money.format(summary.wholeValue)}</strong></div><div><span>Components</span><strong>{money.format(summary.componentValue)}</strong></div><div><span>Set premium</span><strong>{money.format(summary.premium)}</strong></div></div>
      <div className="collectionCompletion"><i style={{ width: `${summary.completionPercent}%` }} /></div><div className="collectionDashboardMeta"><span>{summary.completionPercent}% complete</span><span>{summary.ownedComponents}{summary.expectedComponents ? ` of ${summary.expectedComponents}` : ""} components</span></div>
