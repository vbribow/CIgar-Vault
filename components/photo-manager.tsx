...
export function PhotoManager({ item }: { item: InventoryItem }) {
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(`/api/inventory/${encodeURIComponent(item.inventoryId)}/photos`, {
        method: "POST",
        body: form,
      ...
      if (error instanceof DOMException && error.name === "AbortError") {
        setMessage("The upload took too long. Refresh this record to check whether it finished before trying again.");
      } else {
  ...
  return <section className="section photoSection"><div className="sectionHead"><div><div className="eyebrow">Visual provenance</div><h2>Photos & documents</h2></div></div>
    ...
    <form className="photoUpload" onSubmit={upload} aria-busy={uploading}><label><span>Photo type</span><select name="kind" disabled={uploading}>{kinds.map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label><label><span>Choose or take photo</span><input name="file" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif,application/pdf" required disabled={uploading} /></label><button className="button" disabled={uploading}>{uploading ? "Uploading & syncing…" : "Attach file"}</button>{message && <output aria-live="polite">{message}</output>}</form>
