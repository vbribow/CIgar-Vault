...
test("Hojavía lounge ranking rewards credible samples instead of one perfect visit",()=>{assert.ok(communityPlaceRankingScore(92,40)!>communityPlaceRankingScore(100,1)!);const ranked=rankPlaces([{googlePlaceId:"ONE",name:"One",address:"",googleMapsUri:"https://maps.example/one",cedrivaScore:100,cedrivaReviewCount:1},{googlePlaceId:"MANY",name:"Many",address:"",googleMapsUri:"https://maps.example/many",cedrivaScore:92,cedrivaReviewCount:40}]);assert.equal(ranked[0].googlePlaceId,"MANY")});
test("certification is dated, revisitable, and independently disclosed",()=>{const value=PlaceCertificationInput.parse({googlePlaceId:"PLACE-1",level:"Cedriva Destination",score:96,visitMonth:"2026-07",summary:"An exceptional destination with rigorous humidor care and genuine hospitality.",strengths:"Deep selection and knowledgeable service.",complimentaryDisclosure:"Nothing complimentary.",nextReviewDate:"2027-07-01"});assert.equal(value.level,"Cedriva Destination")});
