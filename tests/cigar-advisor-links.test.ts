...
test("inventory advisor links preserve exact inventory identity and collector intent", () => {
  const url = new URL(cigarAdvisorHref(item, "aging"), "https://hojavia.example");
...
test("inventory records expose aging, pairing, value, and collection guidance", () => {
  ...
  assert.match(new URL(actions[2].href, "https://hojavia.example").searchParams.get("question") || "", /completed-sale evidence/);
  assert.match(new URL(actions[3].href, "https://hojavia.example").searchParams.get("question") || "", /linked collection/);
});

test("complete assessment remains the primary inventory action", () => {
  const question = new URL(cigarAdvisorHref(item), "https://hojavia.example").searchParams.get("question") || "";
