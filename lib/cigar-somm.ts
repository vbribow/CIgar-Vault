...
export const CigarSommQuestionSchema=z.object({question:z.string().trim().min(3).max(1000),inventoryId:z.string().trim().max(100).optional(),occasion:z.string().trim().max(120).optional(),includeAlcohol:z.boolean().default(true)}).strict();
const Pairing=z.object({name:z.string(),style:z.string(),why:z.string(),service:z.string()});
export const CigarSommAnswerSchema=z.object({answer:z.string(),cigarContext:z.string(),confidence:z.enum(["High","Medium","Developing"]),basis:z.array(z.string()).max(6),coffee:z.array(Pairing).max(3),spirits:z.array(Pairing).max(3),cocktails:z.array(Pairing).max(3),nonAlcoholic:z.array(Pairing).max(3),cautions:z.array(z.string()).max(4)});
...
export const cigarSommJsonSchema={type:"object",additionalProperties:false,properties:{answer:{type:"string"},cigarContext:{type:"string"},confidence:{type:"string",enum:["High","Medium","Developing"]},basis:{type:"array",maxItems:6,items:{type:"string"}},coffee:pairingJson,spirits:pairingJson,cocktails:pairingJson,nonAlcoholic:pairingJson,cautions:{type:"array",maxItems:4,items:{type:"string"}}},required:["answer","cigarContext","confidence","basis","coffee","spirits","cocktails","nonAlcoholic","cautions"]} as const;

export async function askCigarSomm(input:z.infer<typeof CigarSommQuestionSchema>,inventory:InventoryItem[],smokes:SmokingLog[]){const apiKey=process.env.OPENAI_API_KEY?.trim();if(!apiKey)throw new Error("Cigar Somm is not configured");const item=input.inventoryId?inventory.find(value=>value.inventoryId===input.inventoryId):undefined;const tastings=item?smokes.filter(value=>value.inventoryId===item.inventoryId).slice(-5):[];const context=item?{inventoryId:item.inventoryId,brand:item.brand,line:item.line,vitola:item.vitola,vintage:item.vintage,quantity:item.currentQty,status:item.status,personalScore:item.score,notes:item.notes,tastings:tastings.map(value=>({overall:value.overall,strength:value.strength,sweetness:value.sweetness,flavor:value.flavor,notes:value.tastingNotes,buyAgain:value.buyAgain}))}:undefined;const prompt=`You are Cigar Somm, Cigar Vault's premium cigar advisor. Answer the collector's question using their supplied inventory and tasting context when available. Be refined, concise, useful, and candid about uncertainty.
...
Selected cigar context: ${JSON.stringify(context||"none selected")}`;const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:process.env.OPENAI_SOMM_MODEL?.trim()||"gpt-5-mini",reasoning:{effort:"low"},store:false,max_output_tokens:2200,input:prompt,text:{format:{type:"json_schema",name:"cigar_somm_answer",strict:true,schema:cigarSommJsonSchema}}}),signal:AbortSignal.timeout(60_000)});const payload=await response.json();if(!response.ok)throw new Error((payload as{error?:{message?:string}}).error?.message||`OpenAI request failed (${response.status})`);const text=responseOutputText(payload);if(!text)throw new Error("Cigar Somm returned no answer");return CigarSommAnswerSchema.parse(JSON.parse(text))}
