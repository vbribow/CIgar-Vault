...
import type { CatalogCigar, CigarCollection, Humidor, HumidorReading, InventoryActivity, InventoryItem, SmokingLog, Valuation } from "./types";
...
export async function getHumidorReadings():Promise<HumidorReading[]>{const sheet=await recordSheet("SMARTSHEET_HUMIDOR_READINGS_SHEET_ID");return sheet.rows.map(row=>{const v=recordValues(row,sheet.columns);return{readingId:String(v.get("Reading ID")||row.id),humidorId:String(v.get("Humidor ID")||""),recordedAt:String(v.get("Recorded At")||""),temperatureF:Number(v.get("Temperature F")||0),humidity:Number(v.get("Humidity RH")||0),source:v.get("Source") as string|undefined,notes:v.get("Notes") as string|undefined};}).sort((a,b)=>b.recordedAt.localeCompare(a.recordedAt));}
...
export async function addHumidorReading(value:Omit<HumidorReading,"readingId">):Promise<HumidorReading>{const sheet=await recordSheet("SMARTSHEET_HUMIDOR_READINGS_SHEET_ID");const result={...value,readingId:`READ-${crypto.randomUUID()}`};const cells=recordCells([["Reading ID",result.readingId],["Humidor ID",result.humidorId],["Recorded At",result.recordedAt],["Temperature F",result.temperatureF],["Humidity RH",result.humidity],["Source",result.source],["Notes",result.notes]],sheet.columns);await request(`/sheets/${requireEnv("SMARTSHEET_HUMIDOR_READINGS_SHEET_ID")}/rows`,{method:"POST",body:JSON.stringify([{toBottom:true,cells}])});return result;}
