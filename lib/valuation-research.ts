...
export async function researchInventoryValuation(item:InventoryItem){
  ...
  const prompt=`Research current pricing evidence for this cigar inventory lot: ${item.brand}; line ${item.line}; vitola ${item.vitola}; vintage/release ${item.vintage??"unknown"}; packaging ${item.packaging??"unknown"}; collector currently owns ${item.currentQty??"an unknown number of"} cigars. The collector's current owned quantity is inventory balance only. Never treat it as an original box count, presentation size, packaging fact, or manufacturer configuration. Find up to three direct, current comparable pages. Return USD values per individual cigar, never box totals. Normalize box prices by the documented source's original cigar count. When the product is a collection sold with a humidor, do not assign the complete-set price to a cigar: research the included cigars individually. Cedriva calculates the residual humidor value separately as documented complete-set retail minus the documented retail of every originally included cigar. replacementValue is current retail replacement price from a manufacturer or established retailer. marketValue is the current aftermarket value: a defensible secondary-market estimate that may use current listings but remains distinct from a completed sale. Separately return lastSaleValue, lastSaleDate, lastSaleVenue, and lastSaleSourceUrl only when a source proves a completed auction or secondary-market transaction for the exact cigar identity; never treat an asking price, open auction, estimate, or unavailable lot as a completed sale. Normalize a completed lot total per cigar. Use null for every lastSale field when no verified completed sale is found. For Habanos, Cuban, vintage, discontinued, or collector presentations, prioritize completed-result archives from established European auction houses over dealer asking prices. Confirm sold status, exact vitola, vintage or box code when available, lot quantity, condition, currency conversion, and whether the published result includes buyer's premium; explain these factors in notes. Do not invent a value when packaging or vintage prevents a clean comparison: use null and explain. source/sourceUrl identify the strongest general price comparable. evidenceDate must be today's date in YYYY-MM-DD. Keep notes under 500 characters and each comparable note under 180 characters. This is research evidence, not an appraisal.`;
