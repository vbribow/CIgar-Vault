...
export default async function PartnerJoinPage({ searchParams }: {
  ...
  return <main className="partnerJoin">
    <section className="partnerJoinStory">
      <a className="appBrand" href="/"><CedrivaMark/><span><strong>Cedriva</strong><small>Preserve · Honor · Grow</small></span></a>
      <div>
        <div className="eyebrow">{partnerName ? `A personal invitation from ${partnerName}` : "Welcome to Cedriva"}</div>
        <h1>Document the collection. Understand the culture. Preserve the story.</h1>
        <p className="lede">Cedriva grows with you—from the first premium cigar to a collection built for generations.</p>
      </div>
    ...
    <section className="partnerJoinAction">
      ...
      <h2>Begin your Cedriva journey</h2>
      ...
      <div className="partnerBenefits">
        ...
        <article><strong>Receive personal guidance</strong><span>Recommendations become more useful as Cedriva learns your palate and goals.</span></article>
      </div>
      <a className="button" href="/login?mode=signup">Create your Cedriva home</a>
      <small className="partnerDisclosure">{partner?.disclosure_text || "Cedriva may compensate participating partners for eligible paid memberships. Partner relationships never determine which cigar information is included or how it is presented."}</small>
    </section>
  </main>;
}
