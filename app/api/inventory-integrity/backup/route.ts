...
export async function GET(request: Request) {
  ...
  try {
    ...
    const payload = JSON.stringify({ format: "hojavia-inventory-backup", version: 2, scope, createdAt: timestamp, recordCount: records.length, records }, null, 2);
    return new NextResponse(payload, { headers: { "content-type": "application/json; charset=utf-8", "content-disposition": `attachment; filename="hojavia-${scope}-${timestamp.slice(0, 10)}.json"`, "cache-control": "no-store" } });
  } catch (error) {
