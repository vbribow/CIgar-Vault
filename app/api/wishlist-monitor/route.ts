...
export async function GET(request:Request){
  ...
  try{
    ...
    const{data,error}=await admin.from("vault_records").select("user_id,record_id,payload,updated_at").eq("kind","wishlist").order("updated_at",{ascending:true}).limit(100);
