...
export async function GET(request:Request){
  ...
  try{
    ...
    const[{data,error},{data:preferences,error:preferencesError}]=await Promise.all([admin.from("vault_records").select("user_id,record_id,payload,updated_at").eq("kind","wishlist").order("updated_at",{ascending:true}).limit(100),admin.from("account_preferences").select("user_id,email_notifications,wishlist_alerts")]);
