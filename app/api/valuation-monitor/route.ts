...
export async function GET(request:Request){
  ...
  try{
    ...
    const requested=Number(new URL(request.url).searchParams.get("limit"));const batchSize=Number.isInteger(requested)&&requested>0?Math.min(requested,valuationBatchSize()):valuationBatchSize(),due=eligible.slice(0,batchSize);
    const outcomes=await inValuationBatches(due,async row=>{try{const research=await researchInventoryValuation(row.item);if(research.marketValue===null&&research.replacementValue===null)return{inventoryId:row.item.inventoryId,status:"unsupported"};const valuation:Valuation={valuationId:`VAL-AUTO-${row.item.inventoryId}-${research.evidenceDate}`.slice(0,190),inventoryId:row.item.inventoryId,valuationDate:research.evidenceDate,replacementValue:research.replacementValue??undefined,marketValue:research.marketValue??undefined,source:research.source,sourceUrl:research.sourceUrl||undefined,confidence:research.confidence,notes:`Automated scheduled research. ${research.notes}`};const{error:saveError}=await admin.from("vault_records").upsert({user_id:row.userId,kind:"valuations",record_id:valuation.valuationId,payload:valuation,updated_at:new Date().toISOString()},{onConflict:"user_id,kind,record_id"});if(saveError)throw saveError;if(research.replacementValue!==null){const updated={...row.item,retailValue:research.replacementValue};const{error:inventoryError}=await admin.from("vault_records").update({payload:updated,updated_at:new Date().toISOString()}).eq("user_id",row.userId).eq("kind","inventory").eq("record_id",row.item.inventoryId);if(inventoryError)throw inventoryError}return{inventoryId:row.item.inventoryId,status:"updated",confidence:research.confidence}}catch(error){return{inventoryId:row.item.inventoryId,status:"failed",error:error instanceof Error?error.message:"Failed"}}});
    return NextResponse.json({data:{checked:outcomes.length,batchSize,remainingEligible:Math.max(0,eligible.length-outcomes.length),outcomes}});
