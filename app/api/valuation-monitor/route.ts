...
export async function GET(request:Request){
  ...
  try{
    ...
    const{data,error}=await admin.from("vault_records").select("user_id,kind,record_id,payload").in("kind",["inventory","valuations"]).limit(2000);if(error)throw error;
    ...
    const eligible=[...users.entries()].flatMap(([userId,group])=>group.inventory.filter(item=>valuationNeedsMonitoring(item,group.valuations)).map(item=>({userId,item}))).sort((a,b)=>valuationMonitorPriority(b.item)-valuationMonitorPriority(a.item));
