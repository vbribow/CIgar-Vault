...
import { loadCollections, loadHumidors, loadSensors, loadValuations } from "@/lib/data";
...
export default async function Home() {
  ...
  const [collections, humidors, sensors, valuations, integrityAudits] = mode === "mock"
    ? [[], [], [], [], []]
    : await Promise.all([loadCollections(), loadHumidors(), loadSensors(), loadValuations(), loadAccountRecords<IntegrityAudit>("integrity").then(records => records ?? [])]);
  ...
  return <main className="shell">
    ...
    <Dashboard items={items} onboarding={onboarding} />
  </main>;
}
