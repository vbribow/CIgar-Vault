...
const providers = [
  {
    ...
    description:
      "Bluetooth monitoring with minute-by-minute history and direct CSV export from the Tempi app.",
    href: "https://tempi.fi/",
    action: "Visit Tempi",
    featured: true,
  },
  ...
  },
  {
    ...
    description:
      "Bluetooth and Wi-Fi thermo-hygrometers with app alerts, historical readings, and data export options.",
...
export default async function SensorsPage() {
  ...
  const linkedSensorPush = sensors.filter(
    (sensor) =>
    ...
    <main className="shell">
      <section className="sensorHero">
        ...
        <div className="tempiCallout">
          <strong>Tempi-ready</strong>
          <span>
            Import exported CSV history today. Gateway or mobile-bridge sync can
            use the same ingestion path later.
          </span>
          <a href="https://tempi.fi/support/" target="_blank" rel="noreferrer">
            Tempi export instructions ↗
          </a>
        </div>
      </section>
      ...
      <section className="providerSection" aria-labelledby="provider-heading">
        <div className="providerHeading">
          <div>
            <div className="eyebrow">Compatible ecosystem</div>
            <h2 id="provider-heading">Start with a trusted sensor.</h2>
          </div>
          <p>
            The platform stores every provider in one standard climate record, so
        ...
        <div className="providerGrid">
          {providers.map((provider) => (
            <article
              className={`providerCard ${provider.featured ? "featured" : ""}`}
              key={provider.name}
            >
              <div className="providerMark" aria-hidden="true">
                {provider.monogram}
              </div>
              <div className="providerStatus">{provider.status}</div>
              <h3>{provider.name}</h3>
