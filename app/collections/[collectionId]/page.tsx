...
export default async function CollectionDetailPage({ params }: { params: Promise<{ collectionId: string }> }) {
  ...
  return <main className="shell collectionDetail">
    ...
    <section className="collectionChronology card"><div><div className="eyebrow">Edition chronology</div><h2>{provenance.collectionYear ? `${provenance.collectionYear} collection edition` : "Collection year needs documentation"}</h2><p>{provenance.yearPolicy} When secondary bands differ within one physical lot, record the year-by-count distribution in the lot notes instead of forcing one year onto every cigar.</p></div><div className="chronologyFacts"><article><strong>{provenance.recordedComponentYears}</strong><span>component years recorded</span></article><article><strong>{provenance.unknownComponentYears}</strong><span>component years still unknown</span></article><article><strong>{provenance.identityReview}</strong><span>identities needing review</span></article>{provenance.unknownComponentYears>0&&<a className="button secondary" href={`/inventory?missing=vintage&collectionId=${encodeURIComponent(collection.collectionId)}#inventory-records`}>Document vintage bands →</a>}</div></section>
    ...
    <section className="section card collectionTrustAudit"><div className="sectionHead"><div><div className="eyebrow">Collection trust audit · {trust.score}%</div><h2>{trust.ready?"Collection record is fully documented":"What remains before this record is complete"}</h2><p>Retail, aftermarket evidence, and presentation value remain separate. A collection release year is never treated as an individual cigar production year.</p></div></div><div className="collectionChecklist">{trust.checks.map(check=><article className={check.status==="Verified"?"owned":"missing"} key={check.id}><i>{check.status==="Verified"?"✓":"!"}</i><div><strong>{check.label} · {check.status}</strong><small>{check.detail}</small></div>{check.href&&<a href={check.href}>{check.href.startsWith("http")?"Review source ↗":"Resolve →"}</a>}</article>)}</div></section>
