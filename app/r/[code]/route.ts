...
export async function GET(request: NextRequest, context: { params: Promise<{ code: string }> }) {
  ...
  const { data: campaign } = await admin
    ...
    .select("id,partner_id,code,destination_path,status,starts_at,ends_at,partners(name,status)")
    .eq("code", code.toLowerCase())
    .maybeSingle();
  const partner = campaign?.partners as unknown as { name?: string; status?: string } | null;
  const now = Date.now();
