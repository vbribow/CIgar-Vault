...
export default async function CigarPage({
  ...
  return (
    <main className="shell">
      ...
      <section className="section card" id="value-evidence"><div className="sectionHead"><div><div className="eyebrow">Why this value?</div><h2>{latestValue?.source || "Valuation evidence needed"}</h2><p className="small">{latestValue?`${latestValue.valuationDate} · ${latestValue.confidence || "Unrated"} confidence · ${latestValue.notes || "No evidence note supplied."}`:"Cedriva has not yet saved source-backed price evidence for this exact cigar identity."}</p></div><div className="ctaRow">{latestValue?.sourceUrl&&<a className="button secondary" href={latestValue.sourceUrl} target="_blank" rel="noreferrer">View price source ↗</a>}<a className="button secondary" href={`/valuations?inventoryId=${encodeURIComponent(item.inventoryId)}`}>{latestValue?"Research new evidence":"Research value"}</a><a className="button secondary" href={`/records?inventoryId=${encodeURIComponent(item.inventoryId)}`}>Enter manually</a></div></div><div className="detailStats"><div><span>Retail replacement</span><strong>{item.retailValue === undefined ? "Not researched" : `$${item.retailValue.toLocaleString()} / cigar`}</strong><small>Current replacement cost—not expected resale proceeds</small></div><div><span>Aftermarket estimate</span><strong>{latestValue?.marketValue === undefined ? "Not researched" : `$${latestValue.marketValue.toLocaleString()} / cigar`}</strong><small>Evidence-based estimate; listings are not completed sales</small></div><div><span>Latest completed sale</span><strong>{latestSale?.lastSaleValue === undefined ? "Not found" : `$${latestSale.lastSaleValue.toLocaleString()} / cigar`}</strong><small>{latestSale?.lastSaleDate || "Exact sold evidence required"}</small></div><div><span>Sale venue</span><strong>{latestSale?.lastSaleVenue || "Not documented"}</strong>{latestSale?.lastSaleSourceUrl&&<a className="textLink" href={latestSale.lastSaleSourceUrl} target="_blank" rel="noreferrer">View proof ↗</a>}</div></div></section>
