...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string; checkout?: string }> }) {
  ...
  const [profileResult, preferencesResult, vaultResult, consentResult] = await Promise.all([
    ...
    supabase.from("account_preferences").select("email_notifications,wishlist_alerts,valuation_research,rating_research,collector_25_contributions,product_analytics,upgrade_recommendations").eq("user_id", user.id).maybeSingle(),
