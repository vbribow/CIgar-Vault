...
test("the exact saved record shows a clear mobile-safe confirmation",()=>{
  ...
  assert.match(page,/href="\/inventory\?add=new#mobile-intake">Add another cigar/);
  assert.match(manager,/initialIntakeOpen/);
  assert.match(manager,/startFresh=\{initialIntakeOpen\}/);
