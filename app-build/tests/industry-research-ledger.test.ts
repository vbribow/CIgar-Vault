...
test("ledger is founder-only and has no publication action",()=>{const route=readFileSync(new URL("../app/api/industry-research-ledger/route.ts",import.meta.url),"utf8");const ui=readFileSync(new URL("../components/industry-research-ledger.tsx",import.meta.url),"utf8");assert.match(route,/authorizeWrite/);assert.match(route,/reviewResearchFact/);assert.doesNotMatch(route,/publish/i);assert.match(ui,/cannot publish publicly/i);assert.match(ui,/Research deeper/)});
test("the OpusX pilot is a plain-language three-step review instead of the full commercial dashboard",()=>{const page=readFileSync(new URL("../app/partner-platform/page.tsx",import.meta.url),"utf8");const platform=readFileSync(new URL("../components/partner-platform.tsx",import.meta.url),"utf8");assert.match(page,/Review one source\. Keep only what it proves\./);assert.match(platform,/Read what the source says/);assert.match(platform,/Choose what Hojavía may keep/);assert.match(platform,/Publish later—only after a separate review/);assert.match(platform,/if\(pilotMode\)return/)});
