...
test("account controls fail closed when any authoritative account source fails", () => {
  assert.match(page, /profileResult\.error\s*\|\|\s*preferencesResult\.error\s*\|\|\s*vaultResult\.error\s*\|\|\s*consentResult\.error/);
...
test("the Vault account query is explicitly scoped to the signed-in collector", () => {
  assert.match(page, /\.from\("vault_records"\)\.select\("kind,record_id,payload,updated_at"\)\.eq\("user_id",\s*user\.id\)/);
});
