...
export function UnifiedIntelligenceDashboard({intelligence,briefing,goals}:{intelligence:ReturnType<typeof buildCollectionIntelligence>;briefing:ReturnType<typeof buildDailyBriefing>;goals:ReturnType<typeof buildCollectionGoals>}){
  ...
  const questions=[
    ...
    {question:"What changed?",answer:mover?`${mover.changePercent>0?"+":""}${mover.changePercent}%`:"History is building",detail:mover?`${mover.item.brand} ${mover.item.line} has the largest measured lot movement.`:"Cedriva requires two dated valuations before reporting movement.",href:"/value-history",label:"Review dated movement"},
    {question:"What should I smoke?",answer:smoke?`${smoke.brand} ${smoke.line}`:"More evidence needed",detail:smoke?`${smoke.vitola} · ${smoke.vintage||"year unknown"} · AI-assisted smoking-window candidate.`:"Add release years and tasting evidence to unlock guidance.",href:"/cigar-somm",label:"Consult Cigar Somm"},
