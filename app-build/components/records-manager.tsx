...
export function RecordsManager({ inventory, initialSmokes, initialValuations, mode, selectedInventoryId }: {
  ...
  return <div className="recordsGrid">
    ...
    <section className="card"><h2>Add a valuation</h2><p className="small">Enter per-cigar values. Cedriva multiplies them by your current quantity.</p><form className="recordForm" onSubmit={event => send(event, "valuation")}><label><span>Valuation ID</span><input name="valuationId" required placeholder="VAL-0001" /></label><label><span>Inventory lot</span>{valuationPicker}</label><label><span>Evidence date</span><input name="valuationDate" type="date" required defaultValue={today()} /></label><label><span>Retail replacement / cigar</span><input name="replacementValue" type="number" min="0" step=".01" /></label><label><span>Market value / cigar</span><input name="marketValue" type="number" min="0" step=".01" /></label><label><span>Source name</span><input name="source" placeholder="Retailer, auction, or price guide" /></label><label><span>Source URL</span><input name="sourceUrl" type="url" placeholder="https://…" /></label><label><span>Confidence</span><select name="confidence"><option>High</option><option>Medium</option><option>Low</option></select></label><label><span>Evidence notes</span><textarea name="notes" rows={3} placeholder="Box size, condition, buyer premium, vintage, or comparable details" /></label>{mode === "smartsheet" && <label><span>Founder write key</span><input name="writeKey" type="password" required /></label>}<button className="button" disabled={mode === "mock"}>Save valuation</button></form><div className="recordList"><h3>Recent valuations</h3>{valuations.slice(0, 8).map(value => <div key={value.valuationId}><strong>{value.inventoryId}</strong><span>{value.valuationDate} · ${value.marketValue ?? value.replacementValue ?? 0}</span></div>)}</div></section>
