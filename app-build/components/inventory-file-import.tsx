...
export function InventoryFileImport() {
  ...
  return <section className="card inventoryFileImport">
    <div><div className="eyebrow">Secure inventory migration</div><h2>Import Excel or CSV</h2><p>Upload one `.xlsx` worksheet or CSV. Cedriva rejects macros, formulas, links, embedded objects, and unsupported files. Nothing is saved until you review and confirm.</p></div>
    <form onSubmit={inspect}><input name="file" type="file" accept=".xlsx,.csv,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required/><button className="button" disabled={busy}>{busy ? "Inspecting…" : "Preview inventory"}</button></form>
    ...
    {preview && <div className="importPreview">
      ...
      <button type="button" className="button" disabled={busy || !selected.size || Boolean(batch)} onClick={commit}>Import {selected.size} reviewed rows</button>
