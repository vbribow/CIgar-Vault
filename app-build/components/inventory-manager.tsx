...
import { applyTotalQuantityCorrection, hasInventoryProvenance, hasPhysicalQuantityBreakdown, inventoryCompleteness } from "@/lib/inventory-model";
...
export function InventoryManager({ initialItems, catalog, ratings, collections, humidors, storageLocationsReady, mode, initialQuery = "", initialMissing = "all", initialStorage = "all", initialCollectionId, initialActiveOnly = false }: { initialItems: InventoryItem[]; catalog: CatalogCigar[]; ratings:ProfessionalRating[]; collections:CigarCollection[]; humidors: Humidor[]; storageLocationsReady: boolean; mode: DataMode; initialQuery?: string; initialMissing?: string; initialStorage?: string; initialCollectionId?: string; initialActiveOnly?: boolean }) {
  ...
  const filtered = useMemo(() => scopedItems.filter((item) => {
    ...
    const missingMatch = missing === "all" || (missing === "quantity" && !hasPhysicalQuantityBreakdown(item)) || (missing === "value" && item.retailValue === undefined) || (missing === "vintage" && (item.vintage === undefined || String(item.vintage).trim() === "")) || (missing === "storage" && !item.storageLocationId?.trim()) || (missing === "provenance" && !hasInventoryProvenance(item));
  ...
  function startEditing(item: InventoryItem, focus: "quantity" | "year" | "price" | "storage" | "provenance" | "all" = "all") {
    ...
    window.setTimeout(() => {
      ...
      if (focus === "quantity") (document.querySelector(`.editingEditor input[name="${missing === "quantity" ? "fullBoxQty" : "quickTotal"}"]`) as HTMLInputElement | null)?.focus();
...
<CatalogFields item={formItem} catalog={catalog} /></>}
        ...
        {focusedQuantity&&missing==="all"&&<label className="quantityField quickTotalField"><span>Correct total cigars now</span><input name="quickTotal" type="number" min="0" step="1" placeholder={String(formItem.currentQty??0)} inputMode="numeric" autoFocus/><small>Fastest option for an immediate total correction. This intentionally replaces the box/loose breakdown, which can be documented later in Inventory Audit.</small></label>}
        {focusedQuantity&&missing==="quantity"&&<div className="inventoryQueueNotice wide"><strong>Complete the physical count</strong><span>Enter both full boxes and loose sticks, using 0 where none are owned. Add cigars per box whenever full boxes are greater than 0.</span></div>}
        ...
        {(showAll||focusedStorage)&&<label><span>Storage location</span><select name="storageLocationId" defaultValue={formItem.storageLocationId||""} autoFocus={focusedStorage} disabled={!storageLocationsReady}><option value="">{storageLocationsReady?"Choose a registered humidor or location":"Storage locations temporarily unavailable"}</option>{formItem.storageLocationId&&!humidors.some(humidor=>humidor.humidorId===formItem.storageLocationId)&&<option value={formItem.storageLocationId}>{formItem.storageLocationId} · Current legacy location</option>}{humidors.map(humidor=><option value={humidor.humidorId} key={humidor.humidorId}>{humidor.name}{humidor.location?` · ${humidor.location}`:""}</option>)}</select><small>{storageLocationsReady?"Choose a registered environment so climate readings and insurance records stay connected. Existing legacy locations remain available until deliberately replaced.":"No storage change can be saved until your private location list is available."} <a href="/humidors">Add a new humidor or storage location →</a></small></label>}
        {(showAll||focusedProvenance)&&<><label className="wide"><span>Provenance notes</span><textarea name="provenanceNotes" defaultValue={formItem.provenanceNotes} rows={3} autoFocus={focusedProvenance}/><small>Record only known purchase, custody, receipt, or ownership details. Leave uncertain facts out.</small></label><label className="wide"><span>Receipt or ownership document URL</span><input name="provenanceDocumentLink" type="url" defaultValue={formItem.provenanceDocumentLink} placeholder="https://…"/><small>Add a private receipt, invoice, certificate, or other ownership-document link. Either this document or factual notes completes the provenance audit.</small></label></>}
        {showAll&&<>
