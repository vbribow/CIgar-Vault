...
export function InventoryManager({ initialItems, catalog, ratings, collections, mode, initialMissing = "all", initialStorage = "all" }: { initialItems: InventoryItem[]; catalog: CatalogCigar[]; ratings:ProfessionalRating[]; collections:CigarCollection[]; mode: DataMode; initialMissing?: string; initialStorage?: string }) {
  ...
  const [editMode, setEditMode] = useState<"quantity" | "year" | "all">("all");
  ...
  const collectionNames = useMemo(() => new Map(collections.map(collection => [collection.collectionId, collection.name])), [collections]);
  ...
  function startEditing(item: InventoryItem, focus: "quantity" | "year" | "all" = "all") {
  ...
  return <>
    ...
    <section className="inventoryMobileList" aria-label="Inventory lots">{filtered.map(item=><article key={item.inventoryId}><div><span>{item.vintage?`Production / release year ${item.vintage}`:"Production / release year needed"} · {item.status||"Review"}</span><h3>{item.brand} {item.line}</h3><p>{item.vitola}</p>{item.collectionId&&<a className="inventoryCollectionTag" href={`/collections/${encodeURIComponent(item.collectionId)}`}>Part of {collectionNames.get(item.collectionId)||item.collectionId} →</a>}</div><div className="mobileQuantity"><strong>{item.currentQty??"—"}</strong><span>total cigars</span><small>{item.fullBoxQty??0} box{item.fullBoxQty===1?"":"es"} · {item.looseStickQty??0} loose</small><small>{item.retailValue===undefined?"Retail value needed":`$${item.retailValue.toFixed(2)} / cigar${retailBoxValue(item)===undefined?"":` · $${retailBoxValue(item)!.toFixed(2)} / box`}`}</small></div><div className="mobileLotActions"><button className="button" onClick={()=>startEditing(item,"quantity")}>Fix quantity</button><button className="button secondary" onClick={()=>startEditing(item,"year")}>{item.vintage?"Edit year":"Add year"}</button><a className="button secondary" href={`/valuations?inventoryId=${encodeURIComponent(item.inventoryId)}`}>{item.retailValue===undefined?"Add retail price":"Update retail price"}</a><button className="button secondary" onClick={()=>startEditing(item)}>Edit all details</button><a href={`/inventory/${item.inventoryId}`}>Open record →</a></div></article>)}</section>
    <div className="tableWrap inventoryDesktopTable"><table className="table"><thead><tr><th><input type="checkbox" aria-label="Select visible inventory" checked={filtered.length>0&&filtered.every((item)=>selected.has(item.inventoryId))} onChange={(event)=>setSelected((current)=>{const next=new Set(current);filtered.forEach((item)=>event.target.checked?next.add(item.inventoryId):next.delete(item.inventoryId));return next})}/></th><th>ID</th><th>Cigar</th><th>Year</th><th>Owned</th><th>Total sticks</th><th>Unit retail</th><th>Box retail</th><th>Lot value</th><th>Habanos</th><th>Status</th><th>Personal</th><th>Published</th><th>Complete</th><th /></tr></thead><tbody>{filtered.map((item) => {const published=ratingSummary(ratings,item.inventoryId);return <tr className={selected.has(item.inventoryId)?"selectedRow":""} key={item.inventoryId}>
      <td><input type="checkbox" aria-label={`Select ${item.inventoryId}`} checked={selected.has(item.inventoryId)} onChange={()=>toggleSelected(item.inventoryId)}/></td><td className="small">{item.inventoryId}</td><td><a href={`/inventory/${item.inventoryId}`}><strong>{item.brand}</strong><div className="small">{item.line} · {item.vitola}</div></a>{item.collectionId&&<a className="inventoryCollectionTag" href={`/collections/${encodeURIComponent(item.collectionId)}`}>Part of {collectionNames.get(item.collectionId)||item.collectionId} →</a>}</td><td>{item.vintage || "—"}</td><td className="small">{item.fullBoxQty === undefined && item.looseStickQty === undefined ? "Not split" : <>{item.fullBoxQty ?? 0} box{item.fullBoxQty === 1 ? "" : "es"}<br />{item.looseStickQty ?? 0} loose</>}</td><td>{item.currentQty ?? "—"}</td><td>{item.retailValue===undefined?"—":`$${item.retailValue.toFixed(2)}`}</td><td>{retailBoxValue(item)===undefined?"—":`$${retailBoxValue(item)!.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`}</td><td>{lotRetailValue(item)===undefined?"—":`$${lotRetailValue(item)!.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`}</td><td>{!isCubanInventory(item)?"—":cubanVerificationStatus(item)==="Verified"?<span className="verifyState verify-verified">Verified ✓</span>:<a href="/verification">{cubanVerificationStatus(item)}</a>}</td><td><span className={`statusPill status-${(item.status||"review").toLowerCase()}`}>{item.status || "Review"}</span></td><td>{item.score ?? "—"}</td><td>{published.highest?<a href="/ratings"><strong>{published.highest}</strong><small className="small"> {published.count} source{published.count===1?"":"s"}</small></a>:<a className="textLink" href="/ratings">Research</a>}</td><td><span className="completeness">{inventoryCompleteness(item)}%</span></td>
      <td className="rowActions"><button onClick={() => startEditing(item,"quantity")}>Fix quantity</button><a href={`/valuations?inventoryId=${encodeURIComponent(item.inventoryId)}`}>Set price</a><button onClick={() => startEditing(item)}>Edit all</button>{mode !== "mock" && <button className="danger" onClick={() => remove(item)}>Delete</button>}</td>
