...
export function CollectorWalkthrough() {
  ...
  return <div className="walkthroughShell">
    ...
    <div className="walkthroughProgress" aria-label={`Walkthrough progress: ${progress}%`}>
      <div><span>Step {step + 1} of {steps.length}</span><strong>{progress}%</strong></div>
      <i style={{ width: `${progress}%` }} />
    </div>

    <nav className="walkthroughSteps" aria-label="Collector walkthrough steps">
      {steps.map(([title], index) => <button type="button" key={title} className={index === step ? "active" : index < step ? "complete" : undefined} onClick={() => setStep(index)} aria-current={index === step ? "step" : undefined}><span>{String(index + 1).padStart(2, "0")}</span><strong>{title}</strong></button>)}
    </nav>

    <section className="walkthroughStage" aria-live="polite">
      <header><span>{steps[step][0]}</span><h2>{steps[step][1]}</h2></header>
      ...
      <footer><button type="button" className="button secondary" disabled={step === 0} onClick={() => setStep(Math.max(0, step - 1))}>← Previous</button>{step < steps.length - 1 ? <button type="button" className="button" onClick={() => setStep(step + 1)}>Continue →</button> : <a className="button secondary" href="/verification">Return to my evidence ledger</a>}</footer>
    </section>
  </div>;
}
