...
export function SmokeJournalBrowser({ smokes:initialSmokes, inventory, mode, initialEditSmokeId }: { smokes: SmokingLog[]; inventory: InventoryItem[]; mode:DataMode; initialEditSmokeId?:string }) {
  ...
  const [showAll,setShowAll]=useState(Boolean(initialEditSmokeId));
  ...
  const filtered=Boolean(query||source!=="all"), visible=filtered||showAll?shown:shown.slice(0,5);
  const vaultCount = entries.filter(entry => entry.source === "vault").length;

  return <>
    <section className="journalResults" aria-live="polite">
      <header><div><div className="eyebrow">Private tasting history</div><h2>{filtered?"Matching smokes":showAll?"All logged smokes":"Most recent smokes"}</h2><small>{filtered?`${shown.length} matching entr${shown.length===1?"y":"ies"}`:showAll?`${entries.length} total entries`:`Showing ${Math.min(5,entries.length)} of ${entries.length}`}</small></div><Link className="button" href="/records#log-smoke">Log a smoke</Link></header>
      {visible.map(entry => {
        const content = <>
    ...
    <section className="card journalControls" aria-label="Search smoke journal">
      <label><span>Search all logged smokes</span><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Cigar, flavor, note, date, or Vault ID" /></label>
      ...
      {filtered && <button type="button" className="button secondary" onClick={() => { setQuery(""); setSource("all"); }}>Clear filters</button>}
    </section>
  </>;
}
