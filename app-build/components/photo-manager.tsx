...
export function PhotoManager({ item, onAttached }: { item: InventoryItem; onAttached?: (item: InventoryItem) => void }) {
  ...
  return <section className="section photoSection" id="record-photos"><div className="sectionHead"><div><div className="eyebrow">Visual provenance</div><h2>Photos & documents</h2></div></div>
    {gallery.length ? <div className="photoGallery">{gallery.map(([label,url])=><a href={url} target="_blank" rel="noreferrer" key={label}><div>{url.toLowerCase().includes(".pdf") ? <span className="documentThumb">PDF</span> : <img src={url} loading="lazy" decoding="async" alt={`${item.brand} ${label}`} />}</div><strong>{label}</strong><small>Open original ↗</small></a>)}</div> : <div className="emptyState">No photos attached yet.</div>}
