...
export function PartnerInvitation({token}:{token:string}){
  ...
  if(error)return <div className="invitationState error"><h2>Invitation unavailable</h2><p>{error}</p><a className="button secondary" href="/">Return to {brand.name}</a></div>;
  if(!data)return <div className="invitationState"><h2>Verifying invitation…</h2><p>{brand.name} is confirming the organization, role, and expiration.</p></div>;
