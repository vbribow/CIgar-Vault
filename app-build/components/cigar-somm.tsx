...
import{CigarSommConcierge}from"@/components/cigar-somm-concierge";
...
type SommSource="inventory"|"manual"|"beverage"|"context";
...
export function CigarSomm({inventory,smokes=[],initialInventoryId="",initialCollectionId="",initialCollectionName="",initialQuestion="",initialManualName=""}:{inventory:InventoryItem[];smokes?:SmokingLog[];initialInventoryId?:string;initialCollectionId?:string;initialCollectionName?:string;initialQuestion?:string;initialManualName?:string}){
 ...
 const reverse=source==="beverage"||source==="context";
 ...
 function chooseSource(value:SommSource){setSource(value);setPairingContext("");setQuestion("");setError("");setResult(undefined)}
 ...
 async function analyze(event:FormEvent<HTMLFormElement>){event.preventDefault();if(source==="inventory"&&collectionChoiceRequired&&!collectionChoiceConfirmed){setError("This collection contains multiple cigars. Confirm the exact cigar before Cigar Somm can continue.");document.querySelector(".collectionSommChooser")?.scrollIntoView({behavior:"smooth",block:"start"});return}if(!ready){setError(source==="inventory"?"Choose a cigar from your vault first.":source==="manual"?"Enter the brand, line, and cigar name.":source==="beverage"?"Enter the exact spirit, wine, beer, or cocktail you have.":"Describe your meal, time of day, mood, or occasion.");return}const form=new FormData(event.currentTarget),request=question.trim()||(source==="beverage"?"Choose the best cigars from my collection for this exact drink. Explain the flavor bridge, contrast, intensity, and serving considerations.":source==="context"?"Recommend the best cigars from my collection for this moment and explain why each pairing works.":"Analyze this exact cigar. Give me a conservative tasting profile, expected progression while smoking, and well-matched coffee, spirit, cocktail, and nonalcoholic pairings.");setBusy(true);setError("");setResult(undefined);try{const response=await fetch("/api/cigar-somm",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:request,inventoryId:source==="inventory"?selected?.inventoryId:undefined,cigarName:source==="manual"?manualName.trim():undefined,pairingDirection:reverse?"occasion-to-cigar":"cigar-to-beverage",pairingContext:reverse?pairingContext.trim():undefined,occasion:String(form.get("occasion")||"")||undefined,includeAlcohol:form.get("includeAlcohol")==="on",collectionChoiceConfirmed:source!=="inventory"||collectionChoiceConfirmed}),signal:AbortSignal.timeout(105_000)}),value=await response.json();if(!response.ok)throw new Error(value.error||"Unable to build this pairing");setResult(value.data)}catch(value){setError(value instanceof DOMException&&value.name==="TimeoutError"?"Research took too long to complete. Please try again; your selection is still here.":value instanceof Error?value.message:"Unable to build this pairing")}finally{setBusy(false)}}
 return <>
  <CigarSommConcierge inventory={inventory} smokes={smokes}/>
...
<button type="button" role="tab" aria-selected={source==="beverage"} onClick={()=>chooseSource("beverage")}>My spirit or cocktail → a cigar</button>
<button type="button" role="tab" aria-selected={source==="context"} onClick={()=>chooseSource("context")}>My meal or moment → a cigar</button>
</div>
...
<span>{source==="beverage"?"What spirit or cocktail do you have? *":"What are you eating or what is the moment? *"}</span>
<textarea value={pairingContext} onChange={event=>setPairingContext(event.target.value)} required minLength={3} maxLength={500} rows={3} placeholder={source==="beverage"?"Example: Woodford Reserve Double Oaked, neat—or a rye Manhattan with less vermouth":"Example: After a steak dinner, a quiet late evening, or morning coffee"}/>
<small>{source==="beverage"?"Include the producer and label, cocktail ingredients, proof, sweetness, ice, or garnish when known.":"Try a meal, time of day, mood, celebration, or setting."}</small>
</label>}<label>
...
<button className="button" disabled={busy||!ready}>{busy?`Researching · ${elapsed}s`:source==="beverage"?"Pair this drink with my cigars":source==="context"?"Find cigars for this moment":collectionChoiceRequired&&!collectionChoiceConfirmed?"Choose the exact collection cigar above":"Analyze cigar & pairings"}</button>{busy&&<output className="sommProgress" aria-live="polite">
