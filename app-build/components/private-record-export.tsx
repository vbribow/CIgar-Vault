...
export function PrivateRecordExport() {
  ...
  async function download(){
    ...
    try{
      ...
      if(!response.ok){const result=await response.json().catch(()=>({}));throw new Error(result.error||"The platform could not prepare your private record.")}
      const blob=await response.blob();
      if(!blob.size)throw new Error("The platform prepared an empty file. Please try again.");
      const disposition=response.headers.get("content-disposition")||"";
      const filename=disposition.match(/filename="?([^";]+)"?/i)?.[1]||`private-collector-record-${new Date().toISOString().slice(0,10)}.json`;
