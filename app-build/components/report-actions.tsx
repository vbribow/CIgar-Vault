  ...
  async function downloadPdf(){
    ...
    try{
      ...
      if(!response.ok){
        ...
        throw new Error(result.error||"Hojavía could not prepare the insurance PDF. Please try again.");
      }
      const blob=await response.blob();
      if(!blob.size||blob.type!=="application/pdf")throw new Error("Hojavía received an incomplete PDF. Please try again.");
      const disposition=response.headers.get("content-disposition")||"";
      const filename=disposition.match(/filename="?([^";]+)"?/i)?.[1]||`hojavia-insurance-schedule-${generatedAt.slice(0,10)}.pdf`;
