...
export function AccountChecklistPanel({ items }: { items: AccountChecklistItem[] }) {
  ...
  return <aside className="card onboardingCard" aria-labelledby="account-checklist-title">
    ...
    {items.map(item => <a href={item.href} key={item.label}><span className={item.complete ? "done" : ""}>{item.complete ? "✓" : ""}</span><strong>{item.label}</strong><small>{item.complete ? "Done" : "Continue setup"}</small></a>)}
  </aside>;
}
