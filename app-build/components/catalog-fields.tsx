...
export function CatalogFields({ item, catalog }: { item: InventoryItem; catalog: CatalogCigar[] }) {
  ...
  return <>
    <label><span>Brand *</span><input name="brand" required list="cigar-brand-options" value={brand} onChange={(event) => { setBrand(event.target.value); setLine(""); setVitola("");setResearched([]); }} placeholder="Search or enter a brand" /><datalist id="cigar-brand-options">{brands.map((value) => <option key={value} value={value} />)}</datalist><small>{brands.length} canonical brands. Known names are standardized when saved.</small></label>
    <label><span>Line / Series</span><input name="line" list="cigar-line-options" value={line} onChange={(event) => { setLine(event.target.value); setVitola("");setResearched([]); }} placeholder={brand ? "Choose or enter a line" : "Select a brand first"} /><datalist id="cigar-line-options">{lines.map((value) => <option key={value} value={value} />)}</datalist><small>{brand && lines.length ? `${lines.length} canonical catalog line${lines.length === 1 ? "" : "s"}` : "Enter the exact documented line name."}</small></label>
