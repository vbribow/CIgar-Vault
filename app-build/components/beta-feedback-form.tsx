...
import {
  ...
  type AccountDataRequestKind,
  type BetaFeedbackMode,
...
export function BetaFeedbackForm({
  ...
  initialRequest?: AccountDataRequestKind;
  initialIncident?: LaunchIncidentSeverity;
}) {
  const accountTemplate = accountDataRequestTemplate(initialRequest);
  const incidentTemplate = launchIncidentReportTemplate(initialIncident);
  const requestTemplate = accountTemplate || incidentTemplate;
  const [items, setItems] = useState<Feedback[]>([]);
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  const [mode, setMode] = useState<BetaFeedbackMode>(requestTemplate?.mode || "Issue report");
