    ...
    {stage === "review" && <section className="intakeStage" aria-labelledby="review-stage-title">
      ...
      <div className="photoIntakeLayout">{photos.length > 0 && <div className="reviewPhotos"><div className="photoPreviewGrid">{photos.map((photo) => <img key={photo.url} src={photo.url} alt={photo.name}/>)}</div><small>{photos.length} selected photo{photos.length === 1 ? "" : "s"}</small></div>}
        <form key={`intake-${captureSession}`} onSubmit={createDraft}>
          ...
          <div className="intakePrimaryAction"><button className="button">Save for final review</button><small>You will confirm this record before it is added to your Vault.</small></div>
        </form>
      </div>
    </section>}

    {stage === "saved" && readyForAnother && <section ref={completion} tabIndex={-1} className="intakeCompletion" aria-labelledby="saved-stage-title" role="status" aria-live="polite"><div className="eyebrow">Step 3 of 3 · Ready to confirm</div><h3 id="saved-stage-title">Your work is saved on this screen.</h3><p>Review it below, then add it to your Vault when everything looks right.</p><div><button type="button" className="button" onClick={nextAsset}>Document another cigar</button><a className="button secondary" href="/inventory#inventory-records">Return to Vault</a></div></section>}
