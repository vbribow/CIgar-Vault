...
export function PhotoInventoryIntake({ catalog, inventory, mode, onDraft, onApproved }: { catalog: CatalogCigar[]; inventory: InventoryItem[]; mode: DataMode; onDraft: (draft: InventoryItem) => void; onApproved: (items: InventoryItem[]) => void }) {
  ...
  function createDraft(event: FormEvent<HTMLFormElement>) {
    ...
    if ([fullBoxes, sticks, loose].some((value) => value !== undefined && (!Number.isInteger(value) || value < 0)) || sticks === 0) { showError("Quantities must be whole numbers. Boxes and loose sticks may be 0; cigars per box must be greater than 0."); return; }
    if ((fullBoxes ?? 0) > 0 && !sticks) { showError("Enter cigars per box when the lot includes a full box."); return; }
