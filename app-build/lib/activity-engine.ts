...
import { createServerRecordId } from "./server-record-id";
import { applyTotalQuantityCorrection, normalizeInventory } from "./inventory-model";
...
export function applyActivity(item:InventoryItem,input:ActivityInput,now=new Date()){
  const {submissionId,...activityInput}=input;
  input=activityInput;
  ...
  if(["Purchase","Add sticks"].includes(input.eventType))after=normalizeInventory({...item,fullBoxQty:beforeBoxes+input.boxes,looseStickQty:beforeLoose+input.quantity});
  if(input.eventType==="Correction")after=applyTotalQuantityCorrection(item,input.quantity);
