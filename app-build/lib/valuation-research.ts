...
Match brand, line, vitola, release, packaging, and condition. Never substitute another vitola/year, MSRP, ordinary retail, or a closeout for secondary-market evidence. Check Bond Roberts for an exact completed lot for both Cuban and New World collectible cigars. Accept it only when the direct lot page proves the sold result and documents exact identity, quantity, packaging, sale date, currency, and whether buyer's premium is included; normalize the sold lot total per cigar only when quantity is explicit. A closed lot, reserve-not-met result, open bid, estimate, or asking price is not a completed sale. For New World cigars, use two or more traceable exact specialty-retailer listings for Retail consensus value; one retail listing supports replacementValue only. For Habanos, never use Retail consensus value: prioritize completed-result archives from established auction houses. Confirm whether buyer's premium is included. Classify every comparable. An asking price is not proof—never treat it as a sale. One secondary listing remains an asking price. An estimated range requires at least two independent exact-identity secondary-market signals. For a humidor collection, research cigars individually; calculate residual humidor value separately. If evidence is opaque, say so.
