...
export const CigarSommQuestionSchema=z.object({question:z.string().trim().min(3).max(1000),inventoryId:z.string().trim().max(100).optional(),cigarName:z.string().trim().min(3).max(300).optional(),occasion:z.string().trim().max(120).optional(),includeAlcohol:z.boolean().default(true),collectionChoiceConfirmed:z.boolean().default(false)}).strict().refine(value=>Boolean(value.inventoryId||value.cigarName),{message:"Choose an inventory cigar or enter a cigar name"});
...
export const CigarSommAnswerSchema=z.object({answer:z.string().max(600),cigarContext:z.string(),confidence:z.enum(["High","Medium","Developing"]),personalization:Personalization,tastingProfile:TastingProfile,basis:z.array(z.string()).max(6),coffee:z.array(Pairing).max(3),spirits:z.array(SpiritPairing).max(3),cocktails:z.array(Pairing).max(3),nonAlcoholic:z.array(Pairing).max(3),sources:z.array(ResearchSource).max(8),cautions:z.array(z.string()).max(4)});
