...
export const CigarSommAnswerSchema=z.object({answer:z.string(),cigarContext:z.string(),confidence:z.enum(["High","Medium","Developing"]),personalization:Personalization,tastingProfile:TastingProfile,basis:z.array(z.string()).max(6),coffee:z.array(Pairing).max(3),spirits:z.array(Pairing).max(3),cocktails:z.array(Pairing).max(3),nonAlcoholic:z.array(Pairing).max(3),sources:z.array(ResearchSource).max(8),cautions:z.array(z.string()).max(4)});
