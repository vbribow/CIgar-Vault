...
export function buildSearchResultHref(resultHref: string, originHref: string, query: string) {
  const safeResult = safeInternalHref(resultHref) || "/";
  const url = new URL(safeResult, localOrigin);
  url.searchParams.set(searchReturnParam, buildSearchRestoreHref(originHref, query));
  return `${url.pathname}${url.search}${url.hash}`;
}
...
export function parseRecentSearches(serialized: string | null) {
  ...
  try { return normalizeRecentSearches(JSON.parse(serialized)); }
  catch { return []; }
}
