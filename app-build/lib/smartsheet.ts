...
import { uniqueSensorReadings } from "./sensor-model";
...
export async function getCatalog(): Promise<CatalogCigar[]> {
  ...
  return sheet.rows.map((row) => {
    ...
    return {
      ...
      brandOwner: values.get("Brand Owner") as string | undefined,
      blender: values.get("Blender") as string | undefined,
      wrapper: values.get("Wrapper") as string | undefined,
      binder: values.get("Binder") as string | undefined,
      filler: values.get("Filler") as string | undefined,
      wrapperOrigin: values.get("Wrapper Origin") as string | undefined,
      binderOrigin: values.get("Binder Origin") as string | undefined,
      fillerOrigins: values.get("Filler Origins") as string | undefined,
      dimensions: values.get("Dimensions") as string | undefined,
      strength: values.get("Strength") as string | undefined,
      releaseYear: values.get("Release Year") as string | number | undefined,
      edition: values.get("Edition") as string | undefined,
      packaging: values.get("Packaging") as string | undefined,
      sourceUrl: values.get("Source URL") as string | undefined,
      sourceName: values.get("Source Name") as string | undefined,
      sourceType: values.get("Source Type") as CatalogCigar["sourceType"],
      confidence: values.get("Confidence") as CatalogCigar["confidence"],
      verifiedAt: values.get("Verified At") as string | undefined,
...
export async function addCatalogDiscoveries(items:CatalogCigar[]){if(!items.length)return 0;const sheet=await recordSheet("SMARTSHEET_CATALOG_SHEET_ID");const existing=new Set(sheet.rows.map(row=>catalogFromRow(row,sheet.columns).catalogId));const pending=items.filter(item=>!existing.has(item.catalogId));if(!pending.length)return 0;const rows=pending.map(item=>({toBottom:true,cells:recordCells([["Catalog ID",item.catalogId],["Brand",item.brand],["Line / Series",item.line],["Cigar / Vitola",item.vitola],["Country",item.country],["Factory",item.factory],["Brand Owner",item.brandOwner],["Blender",item.blender],["Wrapper",item.wrapper],["Binder",item.binder],["Filler",item.filler],["Wrapper Origin",item.wrapperOrigin],["Binder Origin",item.binderOrigin],["Filler Origins",item.fillerOrigins],["Dimensions",item.dimensions],["Strength",item.strength],["Release Year",item.releaseYear],["Edition",item.edition],["Packaging",item.packaging],["Source URL",item.sourceUrl],["Source Name",item.sourceName],["Source Type",item.sourceType],["Confidence",item.confidence],["Verified At",item.verifiedAt],["Master Notes",item.masterNotes],["Research Status","Pending review"]],sheet.columns)}));await request(`/sheets/${requireEnv("SMARTSHEET_CATALOG_SHEET_ID")}/rows`,{method:"POST",body:JSON.stringify(rows)});return pending.length}
export async function reviewCatalogDiscoveries(items:CatalogCigar[],status:"Approved"|"Rejected"){if(!items.length)return 0;const sheet=await recordSheet("SMARTSHEET_CATALOG_SHEET_ID");const byId=new Map(sheet.rows.map(row=>[catalogFromRow(row,sheet.columns).catalogId,row]));const updates=items.map(item=>{const row=byId.get(item.catalogId);if(!row)throw new Error(`Catalog discovery ${item.catalogId} was not found`);return{id:row.id,cells:recordCells([["Brand",item.brand],["Line / Series",item.line],["Cigar / Vitola",item.vitola],["Country",item.country],["Factory",item.factory],["Brand Owner",item.brandOwner],["Blender",item.blender],["Wrapper",item.wrapper],["Binder",item.binder],["Filler",item.filler],["Wrapper Origin",item.wrapperOrigin],["Binder Origin",item.binderOrigin],["Filler Origins",item.fillerOrigins],["Dimensions",item.dimensions],["Strength",item.strength],["Release Year",item.releaseYear],["Edition",item.edition],["Packaging",item.packaging],["Source URL",item.sourceUrl],["Source Name",item.sourceName],["Source Type",item.sourceType],["Confidence",item.confidence],["Verified At",item.verifiedAt],["Master Notes",item.masterNotes],["Research Status",status]],sheet.columns)}});await request(`/sheets/${requireEnv("SMARTSHEET_CATALOG_SHEET_ID")}/rows`,{method:"PUT",body:JSON.stringify(updates)});return updates.length}
...
export async function addSmokingLog(log: SmokingLog): Promise<void> {
  ...
  const cells = recordCells([["Smoke ID",log.smokeId],["Inventory ID",log.inventoryId],["Cigar Name",log.cigarName],["Date Smoked",log.dateSmoked],["Production / Vintage Year",log.vintage],["Overall 1–100",log.overall],["Flavor",log.flavor],["Strength",log.strength],["Sweetness",log.sweetness],["Construction",log.construction],["Burn",log.burn],["Tasting Notes",log.tastingNotes],["Buy Again",log.buyAgain]], sheet.columns);
...
export async function recordSmokingLog(log: SmokingLog): Promise<void> {
  ...
  if(log.burn!==undefined)requireRecordColumns(smokingSheet.columns,["Burn"]);
...
const PRESENTATION_INVENTORY_COLUMN = "Presentation Inventory ID";
...
export async function saveCollection(value:CigarCollection,memberIds:string[]):Promise<void>{
  ...
  if(value.presentationInventoryId&&!collectionSheet.columns.some(column=>column.title===PRESENTATION_INVENTORY_COLUMN)){
    throw new Error(`Add the "${PRESENTATION_INVENTORY_COLUMN}" column to the Collections sheet before saving a presentation asset.`);
  }
  const values:Array<[string,RecordValue]>=[["Collection ID",value.collectionId],["Collection Name",value.name],["Maker / Collaboration",value.maker],["Release Year",value.releaseYear],["Edition / Release",value.edition],["Expected Components",value.expectedComponents],["Expected Cigars",value.expectedCigars],["Whole Collection Market Value",value.wholeMarketValue],["Acquisition Cost",value.acquisitionCost],["Valuation Date",value.valuationDate],["Valuation Source",value.valuationSource],["Valuation Source URL",value.valuationSourceUrl],["Status",value.status],["Photo Link",value.photoLink],[PRESENTATION_INVENTORY_COLUMN,value.presentationInventoryId],["Notes",value.notes]];
