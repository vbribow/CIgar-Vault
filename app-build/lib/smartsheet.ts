...
import { consumeInventory, normalizeInventory } from "./inventory-model";
...
const fieldColumns: Array<[keyof InventoryItem, string]> = [
  ...
  ["acquisitionSeller", "Acquisition Seller"], ["acquisitionDate", "Acquisition Date"],
  ["acquisitionSourceUrl", "Acquisition Source URL"], ["acquisitionReceiptLink", "Acquisition Receipt Link"],
  ["purchaseJurisdiction", "Purchase Jurisdiction"], ["habanosVerificationDate", "Habanos Verification Date"],
  ["habanosVerificationResult", "Habanos Verification Result"], ["habanosVerificationEvidenceLink", "Habanos Verification Evidence Link"],
  ["habanosVerificationNotes", "Habanos Verification Notes"],
...
export async function addSmokingLog(log: SmokingLog): Promise<void> {
  ...
  if((log.quantitySmoked??1)!==1)requireRecordColumns(sheet.columns,["Quantity Smoked"]);
...
export async function recordSmokingLog(log: SmokingLog): Promise<void> {
  ...
  if((log.quantitySmoked??1)!==1)requireRecordColumns(smokingSheet.columns,["Quantity Smoked"]);
  ...
  const after = consumeInventory(before,log.quantitySmoked??1);
