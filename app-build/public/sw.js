...
self.addEventListener("message",event=>{if(event.data?.type==="SKIP_WAITING")self.skipWaiting();if(event.data?.type==="GET_RELEASE"&&event.ports?.[0])event.ports[0].postMessage({release:CACHE})});
