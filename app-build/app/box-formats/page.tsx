...
export default async function BoxFormatsPage() {
  ...
  return <main className="shell">
    <nav className="nav"><a className="brand" href="/">Cedriva</a><div className="navLinks"><a href="/inventory">Inventory</a><a href="/valuations">Valuation</a><a href="/verification">Verification</a></div></nav>
    <section className="verifyHero"><div><div className="eyebrow">Packaging intelligence</div><h1>Boxes and sticks, counted correctly.</h1><p className="lede">Cedriva stores full boxes, cigars per box and loose sticks separately, then calculates the total number of cigars automatically.</p></div><div className="verifyScore"><strong>{matched.length}</strong><span>inventory lots matched</span><small>{pending.length} rare or ambiguous lots still need packaging confirmation</small></div></section>
