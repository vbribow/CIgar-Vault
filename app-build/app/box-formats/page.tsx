...
export default async function BoxFormatsPage() {
  ...
  return <main className="shell">
    <nav className="nav"><a className="brand" href="/">{brand.name}</a><div className="navLinks"><a href="/inventory">Inventory</a><a href="/valuations">Valuation</a><a href="/verification">Verification</a></div></nav>
    <section className="verifyHero"><div><div className="eyebrow">Packaging intelligence</div><h1>Boxes and sticks, counted correctly.</h1><p className="lede">The platform stores full boxes, cigars per box and loose sticks separately, then calculates the total number of cigars automatically.</p></div><div className="verifyScore"><strong>{matched.length}</strong><span>inventory lots matched</span><small>{pending.length} rare or ambiguous lots still need packaging confirmation</small></div></section>
    ...
    <section className="section"><div className="sectionHead"><div><div className="eyebrow">Sourced catalog</div><h2>Known box formats</h2></div><a className="button secondary" href="/inventory">Enter quantities</a></div><div className="tableWrap"><table className="table"><thead><tr><th>Brand</th><th>Cigar</th><th>Known box sizes</th><th>Source</th></tr></thead><tbody>{boxFormats.map((format)=><tr key={`${format.brand}-${format.product}`}><td><strong>{format.brand}</strong></td><td>{format.product}</td><td>{format.sizes.join(" or ")} cigars</td><td><a className="textLink" href={format.sourceUrl}>{format.sourceLabel} ↗</a></td></tr>)}</tbody></table></div></section>
