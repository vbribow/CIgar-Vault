...
export const metadata:Metadata={title:"Canonical Provenance Graph",description:"How Cedriva connects cigar identity, manufacturing, release history, artifacts, evidence, and private collector lots."};
...
export default async function ProvenanceGraphPage(){
  ...
  return <main className="shell wideShell provenancePage"><section className="provenanceHero"><div><div className="eyebrow">Cedriva canonical provenance graph</div><h1>Every cigar connects to people, place, time, and evidence.</h1><p className="lede">Cedriva does not flatten a cigar into one inventory row. The graph preserves the product, the work behind it, the artifacts that identify it, and the private story of each collector lot.</p></div><aside><strong>9</strong><span>connected record types</span><p>Shared knowledge stays separate from private ownership, while evidence travels with every claim.</p></aside></section>
    ...
    <section className="provenancePromise"><div className="eyebrow">Permanent design rule</div><h2>The graph may grow. Its history must never collapse.</h2><p>Releases, packaging, manufacturing relationships, evidence, and custody can change over time. Cedriva preserves each dated state so a later correction adds truth without erasing the past.</p></section>
  </main>;
}
