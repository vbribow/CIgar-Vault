...
export default async function BrandResearchPage({ searchParams }: { searchParams: Promise<{ name?: string }> }) {
  ...
  return <main className="shell brandResearchPage">
    <nav className="nav"><a className="brand" href="/">{brand.name}</a><div className="navLinks"><a href="/catalog-discovery#research-backlog">Evidence backlog</a><a href="/learn/manufacturing-truth#research-standard">Evidence standard</a><span className="badge">Founder research</span></div></nav>
    <section className="brandResearchHero">
      <div><div className="eyebrow">Brand research · {item.priority}</div><h1>{item.brand}</h1><p>{item.primaryRegion} · {item.segment}</p><span>{item.status}</span></div>
