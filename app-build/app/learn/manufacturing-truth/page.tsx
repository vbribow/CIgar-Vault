...
const identityLayers = [
  ...
  ["Actual factory", `Processes, bunches, rolls, finishes, rests, and quality-checks the cigar. This is the manufacturing truth ${brand.name} records.`],
...
export default async function ManufacturingTruthPage() {
  ...
  return <main className="shell manufacturingTruthPage">
    ...
    <section className="truthTrustStandard">
      <div><div className="eyebrow">The {brand.name} Trust Framework</div><h2>Evidence remains visible.</h2></div>
    ...
    <section className="truthDirectorySection" id="directory">
      <div className="truthSectionHead"><div><div className="eyebrow">Deep manufacturing systems · {manufacturingTruthRecords.length} verified records</div><h2>Follow the cigar beyond the label.</h2></div><p>These records have enough evidence to connect ownership, creative authorship, actual production, history, and a release-level rule. They supply manufacturing truth to every matching cigar in {brand.name}’s live catalog.</p></div>
    ...
    <section className="truthMethod" id="research-standard">
      <div><div className="eyebrow">How {brand.name} protects the record</div><h2>Corrections strengthen trust.</h2></div>
      <div><p>Each manufacturing claim carries a source type, confidence level, and verification date. When production changes, {brand.name} preserves the previous relationship by release period rather than silently replacing it.</p><p>A brand-level factory is only inherited by a cigar when the evidence supports that relationship. A line, vitola, release year, or market exception can override it. When a factory is undisclosed, disputed, or supported only by indirect evidence, the record says so. Accuracy comes before completeness.</p><div className="ctaRow"><a className="button" href="#all-brands">Search every brand</a><a className="button secondary" href="/learn/factory-map-reading">Learn to read a factory map</a><a className="button secondary" href="/catalog-discovery#research-backlog">Open research backlog</a><a className="button secondary" href="/learn/blending#profiles">Meet the blenders</a></div></div>
    </section>
  </main>;
}
