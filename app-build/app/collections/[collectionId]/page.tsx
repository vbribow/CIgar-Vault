...
export default async function CollectionDetailPage({ params }: { params: Promise<{ collectionId: string }> }) {
  ...
  return <main className="shell collectionDetail">
    ...
    {template&&<aside className="collectionStewardship"><span>Stewardship before speculation</span><strong>Preserve the edition, its makers, and its original presentation.</strong><p>The collection record documents this {template.releaseYear?`${template.releaseYear} `:""}work by {template.maker} from attributable sources. Market evidence is one chapter of the record—not the meaning of the collection.</p><a href={template.sourceUrl} target="_blank" rel="noreferrer">Read the documented source ↗</a></aside>}
