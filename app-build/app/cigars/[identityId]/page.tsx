...
import { claimsUnverifiedCompletedSale, completedSaleLabel, isVerifiedCompletedSale, marketAskingPriceLabel, marketEvidenceType, strongestEvidenceUrl } from "@/lib/valuation-evidence";
...
function valuationPresentation(value:Valuation){
  if(isVerifiedCompletedSale(value))return{label:completedSaleLabel(value),amount:value.lastSaleValue,date:value.lastSaleDate||value.valuationDate,url:strongestEvidenceUrl(value.lastSaleSourceUrl)};
  if(claimsUnverifiedCompletedSale(value))return{label:completedSaleLabel(value),amount:value.lastSaleValue??value.marketValue,date:value.valuationDate,url:strongestEvidenceUrl(value.sourceUrl||value.lastSaleSourceUrl)};
  if(value.askingPrice!==undefined&&value.askingPriceSourceUrl)return{label:marketAskingPriceLabel,amount:value.askingPrice,date:value.valuationDate,url:strongestEvidenceUrl(value.askingPriceSourceUrl)};
  if(value.marketValue!==undefined)return{label:marketEvidenceType(value),amount:value.marketValue,date:value.valuationDate,url:strongestEvidenceUrl(value.sourceUrl)};
  return{label:"Retail replacement evidence",amount:value.replacementValue,date:value.valuationDate,url:strongestEvidenceUrl(value.sourceUrl)};
}
...
export default async function UnifiedCigarStoryPage({ params }: { params: Promise<{ identityId: string }> }) {
  ...
  return <main className="shell cigarStoryPage">
    ...
    <aside className="evidenceLabel" data-level="Community"><span>Evidence status</span><strong>{story.sourceCount ? "Collector record with linked sources" : "Collector record · sources not yet linked"}</strong><small>Each identity, price, review, and journal claim keeps its own evidence label. No single source verifies this entire page. · Commercial influence: none disclosed</small></aside>
    ...
    <section className="storyMetrics">
      ...
      <article><span>Market estimate</span><strong>{money(story.marketUnit)}</strong><small>Per cigar · {story.latestMarketValuation ? marketEvidenceType(story.latestMarketValuation).toLowerCase() : "not a confirmed sale"}</small></article>
      ...
      <article><span>Your average score</span><strong>{story.personalAverage ?? "Not yet rated"}</strong><small>{story.personalScoreCount} scored experience{story.personalScoreCount === 1 ? "" : "s"} · {story.smokes.length} journal entr{story.smokes.length === 1 ? "y" : "ies"}</small></article>
    ...
    <section className="marketMovementSummary" data-direction={movement.direction}><div><div className="eyebrow">{movement.mode==="Same-month comparison"?"Exact-identity monthly price comparison":"Exact-identity monthly market movement"}</div><h2>{movement.mode==="Same-month comparison"?"One month of prices · no trend yet":movement.direction==="Developing"?"A monthly trend is still developing.":`${movement.direction}${movement.changePercent===undefined?"":` · ${Math.abs(movement.changePercent)}%`}`}</h2><p>{movement.summary}</p></div><aside><strong>{movement.confidence}</strong><span>{movement.observationCount} {movement.evidenceType?.toLowerCase()||"dated market"} observation{movement.observationCount===1?"":"s"}</span><small>{movement.monthCount} distinct month{movement.monthCount===1?"":"s"} · {movement.sourceCount} linked source{movement.sourceCount===1?"":"s"} · evidence categories are never mixed</small></aside></section>
    ...
    <section className="storyColumns">
      ...
      <article className="section card"><div className="eyebrow">Evidence ledger</div><h2>{story.valuations.length + story.ratings.length} active connected records</h2>{story.valuations.slice(0, 3).map(item => {const evidence=valuationPresentation(item);const content=<><span><strong>{evidence.label}</strong><small>{evidence.date} · {item.confidence || "Unrated"} confidence{item.source?` · ${item.source}`:""}</small></span><b>{money(evidence.amount)}{evidence.url?" ↗":""}</b></>;return evidence.url?<a className="storyRow" href={evidence.url} key={item.valuationId}>{content}</a>:<div className="storyRow" key={item.valuationId}>{content}</div>})}{story.ratings.slice(0, 3).map(item => <a className="storyRow" href={item.sourceUrl} key={item.ratingId}><span><strong>{item.publication}</strong><small>{item.matchConfidence} identity match</small></span><b>{item.score} ↗</b></a>)}</article>
    </section>
