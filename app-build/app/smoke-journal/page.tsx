...
export default async function SmokeJournalPage() {
  const [smokesResult, inventoryResult] = await Promise.allSettled([loadSmokingLogs(), loadInventory()]);
  const ready = smokesResult.status === "fulfilled" && inventoryResult.status === "fulfilled";
