...
import { filterPlacesByRadius,isConfirmedCigarLoungeCandidate,mergeOfficialLoungeLocations,rankPlaces,type GooglePlaceResult,type PlaceCertification,type PlaceReview,vibeConsensus,communityPlaceScore,normalizeCertificationLevel } from "@/lib/places";
...
export async function GET(request:Request){
 ...
 try{
  ...
  const unique=filterPlacesByRadius(mergeOfficialLoungeLocations(googleResults),radiusMiles);
  ...
  return NextResponse.json({data,meta:{location,radiusMiles,googleAttributionRequired:true,retrievedAt:new Date().toISOString(),dailyLimit:reservation.dailyLimit,searchesUsedToday:reservation.usedToday,methodology:`Results are limited to ${radiusMiles} miles of Google's closest match for your search. Official lounge location pages may fill a branch omitted by Google's result limit. Lounges with at least five community ratings rank first using a sample-size-adjusted community score; Google remains a separate discovery signal.`}},{headers:{"Cache-Control":"private, no-store, max-age=0"}});
 }catch(error){
