...
export async function POST(request:Request){
  ...
  try{
    ...
    const rows=[
      ...
      {user_id:user.id,kind:"integrity" as const,record_id:auditId,payload:{action:"complete-vault-recovery",mode:input.mode,sourceCreatedAt:input.export.createdAt,sourceRecordCount:input.export.recordCount,restored:selected.length,preview,createdAt:restoredAt},updated_at:restoredAt},
    ];
