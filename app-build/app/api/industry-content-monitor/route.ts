...
import { requiredIndustryMediaChecks } from "@/lib/industry-media-sources";
import { z } from "zod";
...
export async function GET(request:Request){
  ...
  try{
    ...
    const prompt=`Today is ${today}. Find up to 12 genuinely newsworthy premium handmade cigar-industry items published during the last 10 days. Rotate across heritage and boutique brands, new releases, growers and producing countries, factories and blenders, auctions, retailers, education, legislation, and advocacy. Use a direct published article, government record, official press release, or named established trade publication. ${requiredIndustryMediaChecks()} Exclude social posts, forums, rumors, duplicate syndication, generic tobacco/vape news with no material premium-cigar impact, SEO pages, stale stories, and promotional restocks. For legal or political matters, accurately distinguish proposal, consultation, enacted law, effective rule, and advocacy position. For advocacy, identify the organization and label its stance; never rewrite its position as neutral fact. Set sourceName to the publication being credited, not the company discussed in the story. Summaries must be original, factual, 35-70 words, and must not copy article prose. Every URL must be a direct page you visited. Return only items worth showing collectors.`;
