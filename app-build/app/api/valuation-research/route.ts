...
export async function GET(request:Request){
 if(!await authorized(request))return NextResponse.json({error:"Sign in before checking valuation research"},{status:401});
 return NextResponse.json({data:{configured:Boolean(process.env.OPENAI_API_KEY?.trim())}});
}
