...
export async function GET(request:Request){
  ...
  try{const inventoryId=new URL(request.url).searchParams.get("inventoryId");if(!inventoryId)return NextResponse.json({data:[]});const{data,error}=await admin().from("retailer_purchase_sessions").select("id,retailer_key,retailer_name,status,purchase_date,receipt_verified_at").eq("user_id",user.id).eq("inventory_id",inventoryId).order("created_at",{ascending:false}).limit(30);if(error)throw error;return NextResponse.json({data:data||[]})}catch{return NextResponse.json({data:[]})}
}
