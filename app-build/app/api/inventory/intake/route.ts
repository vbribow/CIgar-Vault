...
export async function POST(request:Request){
  try{
    ...
    const [masterInventory,masterValuations,accountValuations]=await Promise.all([
      getInventory().catch(()=>[]),
      getValuations().catch(()=>[]),
      signedIn?loadAccountRecords<Valuation>("valuations").then(value=>value||[]):Promise.resolve([]),
    ]);
    const current=account??masterInventory;
    const immediate=applyReusableValuations(submitted,[...current,...masterInventory],[...accountValuations,...masterValuations]);
    const drafts=immediate.items;
    ...
    const accountSaved=signedIn?await importOwnedRecords([...drafts.map(payload=>({kind:"inventory" as const,recordId:payload.inventoryId,payload})),...immediate.valuations.map(payload=>({kind:"valuations" as const,recordId:payload.valuationId,payload}))]):0;
    if(!signedIn&&!input.syncMaster&&dataMode()!=="mock"){masterSaved=await addInventoryRows(drafts);await Promise.all(immediate.valuations.map(recordValuation))}
    return NextResponse.json({data:{approved:drafts.length,accountSaved,masterSaved,synchronized:input.syncMaster&&accountSaved>=drafts.length&&masterSaved===drafts.length,valuedImmediately:immediate.valuedImmediately,valuationStatus:immediate.valuedImmediately===drafts.length?"Every approved cigar received a current exact-match value.":"Unmatched cigars entered the priority research queue.",inventory:drafts}},{status:201});
  }catch(error){
