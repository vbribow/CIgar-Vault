...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<{ missing?: string; storage?: string; collectionId?: string; active?: string }> }) {
  ...
  return <main className="shell">
    ...
    {presentationAssetCount>0&&<section className="card inventoryDataNotice"><div><strong>{presentationAssetCount} presentation asset{presentationAssetCount===1?" is":"s are"} tracked separately</strong><p>Collectible humidors and presentation pieces remain protected in Valuable Collections and are not counted as individual cigars.</p></div><Link className="button secondary" href="/collections" prefetch>Open Valuable Collections</Link></section>}
    ...
    <div><InventoryManager initialItems={cigarItems} catalog={catalog} ratings={ratings} collections={collections} mode={mode} initialMissing={filters.missing} initialStorage={filters.storage} initialCollectionId={filters.collectionId} initialActiveOnly={filters.active === "1"} /></div>
  </main>;
}
