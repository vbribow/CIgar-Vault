...
import { loadCollections, loadHumidors, loadSmokingLogs } from "@/lib/data";
...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<{ missing?: string; storage?: string; status?: string; collectionId?: string; active?: string; vaultSearch?: string; inventoryId?:string; edit?: string; focus?: string;cigarName?:string;add?:string }> }) {
  ...
  const [collectionsResult, humidorsResult, smokesResult] = await Promise.allSettled([loadCollections(), mode === "mock" ? Promise.resolve([]) : loadHumidors(), loadSmokingLogs()]);
  ...
  return <main className="shell">
    ...
    <nav className="vaultPaths" aria-label="Vault workspaces">
      ...
      <Link href="/humidors"><span>Storage and climate</span><strong>Humidors</strong><small>Review conditions, alerts, sensor history, and the cigars assigned to each environment.</small><b>{humidorsResult.status==="fulfilled"?`${humidors.length} humidor${humidors.length===1?"":"s"} →`:"Records unavailable →"}</b></Link>
      <Link href="/collection-health#finish-my-vault"><span>Guided record review</span><strong>Finish My Vault</strong><small>Review one cigar and one useful next step at a time.</small><b>Continue review →</b></Link>
    ...
    <div><InventoryManager initialItems={cigarItems} catalog={[]} ratings={[]} smokes={smokes} collections={collections} humidors={humidors} mode={mode} initialMissing={filters.missing} initialStorage={filters.storage} initialStatus={filters.status} initialCollectionId={filters.collectionId} initialActiveOnly={filters.active === "1"} initialQuery={filters.vaultSearch||filters.inventoryId} initialEditId={filters.edit} initialEditMode={editFocus} initialIntakeQuery={filters.cigarName} initialIntakeOpen={filters.add === "new"}/></div>
  </main>;
}
