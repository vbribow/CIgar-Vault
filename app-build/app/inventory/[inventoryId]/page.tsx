...
import { inventoryOrigin } from "@/lib/inventory-origin";
...
export default async function CigarPage({
  ...
  const origin=catalogReady?inventoryOrigin(item,catalog):{status:"Research needed" as const,reason:"Catalog information is temporarily unavailable."};
  return (
    <main className="shell">
      ...
      {query.saved==="inventory"&&<section className="inventorySavedConfirmation" role="status" aria-live="polite" aria-atomic="true"><div><div className="eyebrow">Save complete</div><strong>Saved to your private Vault</strong><p>{item.brand} {item.line} is ready below. This is the exact record that was saved.</p></div><div className="inventorySavedActions"><Link href="/inventory?add=new#mobile-intake">Add another cigar</Link><Link href={backHref}>Return to Vault</Link></div></section>}
      ...
      {!isPresentationAsset&&<section className="section card inventoryOrigin"><div><div className="eyebrow">Cigar origin</div><h2>{origin.country||"Origin research needed"}</h2><p>{origin.reason}</p></div>{origin.catalog?<Link className="button secondary" href={canonicalCatalogHref(origin.catalog.catalogId)}>Open cigar evidence</Link>:<Link className="button secondary" href={`/discover?query=${encodeURIComponent(`${item.brand} ${item.line} ${item.vitola}`)}`}>Research this exact cigar</Link>}</section>}
