...
export default function PrivacyPage() {
  return <main className="shell legalPage"><div className="eyebrow">Effective July 24, 2026 · Beta version 1.0</div><h1>{brand.name} Privacy Notice</h1><p className="lede">Your collection is private by default. {brand.name} should earn insight without taking ownership of your story.</p>
    ...
    <section><h2>Retailer and commercial separation</h2><p>Retailers do not receive access to your Vault, searches, wishlist, valuations, provenance, or purchase history. Any future retailer-submitted availability record belongs to a separate commercial dataset with its source, date, review state, and material relationship visible; it does not alter your private records.</p></section>
    <section><h2>Control and recovery</h2><p>You may export your complete vault from Account, restore it through the recovery workflow, change privacy preferences, and submit a signed-in request for access, correction, or deletion. Deletion requests are reviewed for identity, scope, required retention, export options, timing, and irreversible effects before any data is removed. <a href="/feedback?request=deletion">Start a private deletion request.</a></p></section>
