...
export default async function VerificationPage() {
  ...
  return <main className="shell">
    <nav className="nav"><a className="brand" href="/">{brand.name}</a><div className="navLinks"><a href="/inventory">Inventory</a><a href="/valuations">Valuation</a><a href="/collection-health">Collection health</a><a href="/storage">Storage</a></div></nav>
    <section className="verifyHero"><div><div className="eyebrow">Cuban verification</div><h1>Verify every boxed Cuban lot.</h1><p className="lede">Record the box code and seal photo, check the code with Habanos, then mark the lot verified in your Vault.</p><div className="verifyActions"><a className="button" href={HABANOS_CHECKER}>Open official Habanos checker →</a><a className="button secondary" href="/inventory">Update inventory</a></div></div><div className="verifyScore"><strong>{groups.verified.length}</strong><span>verified Cuban lots</span><small>{groups.ready.length} ready for the official check</small></div></section>
